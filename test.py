...
def main():
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader):
            ...
            val_outputs_copy= np.squeeze(val_outputs, axis=0)
            
            
            img = nib.Nifti1Image(val_outputs_copy, np.eye(4))
            nib.save(img, os.path.join('results_test/', img_name))
            ...
            for i in range(1, 3):
                ...
                print(organ_Dice)
                if i == 1:
                    dice_liver_all.append(organ_Dice)
                if i == 2:
                    dice_spleen_all.append(organ_Dice)
          
        print("Overal Mean Dice of liver: {}".format(np.mean(dice_liver_all)))
        print("Overal Mean Dice of Spleen: {}".format(np.mean(dice_spleen_all)))
        print(np.std(dice_liver_all))
        print(np.std(dice_spleen_all))
