    ...
    with torch.no_grad():
        ...
        for idx, batch_data in enumerate(loader):
            ...
            if args.distributed:
                ...
                # print(acc_list)
                # avg_acc = np.mean([np.nanmean(l) for l in acc_list])
                avg_acc = np.mean([np.nanmean(l[1:]) for l in acc_list])
                # print(avg_acc)
            val_acc_all.append(avg_acc)
            ...
            start_time = time.time()
        # print(val_acc_all)
        real_val_acc_avg = np.mean(val_acc_all)
    # return avg_acc
    return real_val_acc_avg
